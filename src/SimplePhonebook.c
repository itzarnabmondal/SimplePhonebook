...
void namefun() {
  ...
  double phone;
  char gender[8];
  fptr = fopen("info.txt", "ab+"); //ab+ gives us the ability of writing the function and add the second data in the existing one...
  if (fptr == NULL) {
  ...
  char ch;
  gotoxy(31, 4);
  printf("If you want to add more press 'y':");
  Sleep(1000);
  fflush(stdin);
  while ((ch = getch()) == 'y') {
    menu();
